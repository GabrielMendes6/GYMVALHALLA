...
use GuzzleHttp\Psr7\Header;
    ...
    if ($_SERVER["REQUEST_METHOD"] === "POST") {
        //Verifica se o Formulario de Imagens foi enviado
        if (isset($_FILES["file"]) && $_FILES["file"]["error"] === UPLOAD_ERR_OK) {
            //Pasta dos Arquivos que serão salvos
            $dir = "../uploads/";
        ...
        if (isset($_POST["email"]) && $_POST["email"] != $email) {
            ...
            if (filter_var($newEmail, FILTER_VALIDATE_EMAIL)) {
                ...
                $userModel = new User();
                $userModel->storeEmailVerificationToken($user_id, $newEmail, $token);
    
                if ($userModel->sendVerificationEmail($newEmail, $token)) {
                    Header("location: ../email/emailEnv.php");
                } else {
                    $emailEnvfail = 1;
                }
            } else {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["tel"]) && $_POST["tel" ] !== $tel) {
            $newPhoneNumber = filter_var($_POST["tel"], FILTER_SANITIZE_NUMBER_INT); // Número de telefone do destinatário
            $verificationCode = rand(100000, 999999); // Código de verificação gerado dinamicamente ou por lógica adicional
            $user_id = $_SESSION["user_id"];
            // Chamada da função para enviar SMS
            $userModel = new User();
            $userModel->storeTelVerificationCod($user_id, $newPhoneNumber, $verificationCode);

            if ($userModel->sendVerificationTel($newPhoneNumber, $verificationCode)) {
                Header("Location: ../phone/telEnv.php");
            } else {
...
<head>
    ...
    <link rel="stylesheet" href="../identity/person.css">

    <script src="../src/main.js" defer></script>
    <script src="../identity/person.js" defer></script>
    <title id="title">GYMVALHALLA | LOGIN</title>
</head>
<body>
                    ...
                    <form action="" method="post" enctype="multipart/form-data">
                                        ...
                                        <td class="Itens">
                                            <div>
                                                <div class="inputFPerson">
                                                    ...
                                                    <?php if (isset($emailEnvfail) && $emailEnvfail == 1) : ?>
                                                        <p class="emailEnv">Falha ao Enviar seu E-mail.</p>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                        </td>
                        ...
                        <div id="ChooseImg">
                            <div id="ContainerImgP">
                                <div id="BoxImg">
                                    <?php if (isset($imgPerfil) && !empty($imgPerfil)) : ?>
                                        <img id="previewImg" src="<?php echo $imgPerfil; ?>" alt="Imagem de Perfil">
                                    <?php else: ?>
                                        <img id="previewImg" src="../img/Account/perfil.jpg" alt="Imagem de Perfil">
                                    <?php endif; ?>
                                </div>
                                <div id="divCBtn">
                                ...
                                <div id="infoTextPimg">
                                    <p class="styminFontP">Tamanho do arquivo: no máximo 1 MB</p>
                                    <p class="styminFontP">Extensão de arquivo: .JPEG, .PNG</p>
                                </div>
                                </div>
                            </div>        
                    </form>
